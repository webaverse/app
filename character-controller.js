    ...
    return function(position, quaternion, {relation = 'floor'} = {}) {
      ...
      if (renderer.xr.getSession()) {
        ...
        camPos.copy(position)
          .sub(localVector2.copy(cameraManager.getCameraOffset()).applyQuaternion(camera.quaternion)).add(new Vector3(0, relation === 'floor' ? avatarHeight : 0, 0));
        camera.matrix.setPosition(camPos);
        // camera.position.y += relation === 'floor' ? avatarHeight : 0;
        // camera.quaternion.copy(quaternion);
        camera.matrix.makeRotationFromQuaternion(quaternion);
        // camera.updateMatrix();
