...
export default function Chains() {
  const {chains, selectedChain, selectChain, isChainSupported} = useContext(ChainContext);

  const [open, setOpen] = useState(false);
  return (<div style={{background: selectedChain.brandColor || 'black'}} className={styles.chainSelector}>
    <div className={styles.selectedChain} onClick={() => setOpen(!open)}>{open ? <UpArrow /> : <DownArrow />} {selectedChain.name}</div>
