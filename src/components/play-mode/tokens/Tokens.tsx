...
export const Tokens = ({ loginMethod, loginAddress }) => {
    ...
    useEffect( () => {
        ...
        if ( loginAddress && ! nfts && loginMethod ) {
            ...
            ( async () => {
                ...
                if ( loginMethod === 'metamask' ) {
                    ...
                    const res = await fetch(`https://api.opensea.io/api/v1/assets?owner=${ loginAddress }&limit=${ 50 }`, { headers: { 'X-API-KEY': '6a7ceb45f3c44c84be65779ad2907046' } });
                    const json = await res.json();
                    setNfts( json.assets );

                } else if ( loginMethod === 'discord' ) {

                    let res = await fetch( `${ tokensHost }/${ loginAddress }` );
                    res = await res.json();
    ...
    return (
        <section className={ classnames( styles.sidebar, state.openedPanel === 'UserPanel' ? styles.open : null ) } onClick={ stopPropagation } >
            {
                ( nfts || [] ).map( ( nft, i ) => {
                    ...
                    return (
                        <div className={styles.nft} onDragStart={ e => { e.dataTransfer.setData('application/json', JSON.stringify( nft ) ); }} draggable key={ i } >
