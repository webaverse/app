...
import { makeRng, voxelPixelSize, chunkWorldSize, placeNames, createMapChunk, createMapChunkMesh } from '../../../../procgen/procgen.js';

import { MiniHup } from '../../play-mode/mini-hup';
...
const _makeChunkMesh = ( x, y ) => {
...

    {
        ...
        textMesh.sync( () => {
            ...
            w += 1;
            h += 1;
            labelMesh.position.set( x - chunkWorldSize / 2 + w / 2, 1, y + chunkWorldSize / 2 - h / 2 );
        ...
        textMesh.position.set( - chunkWorldSize / 2 + textOffset, 1, chunkWorldSize / 2 - textOffset );
...

    {
        ...
        const labelGeometry = new THREE.PlaneBufferGeometry( 1, 1 ).applyMatrix4(
            new THREE.Matrix4().makeRotationFromQuaternion( new THREE.Quaternion().setFromAxisAngle( new THREE.Vector3( 1, 0, 0 ), - Math.PI / 2 ) )
        );
...
export const MapGen = () => {
    ...
    const getChunksInRange = () => {
        ...
        const chunks = [];
        const bottomLeft = localVectorX.set( - 1, 1, 0 ).unproject( camera );
        const topRight = localVectorX2.set( 1, - 1, 0 ).unproject( camera );
    ...
    useEffect( () => {
        ...
        const mouseMove = ( e ) => {
            ...
            if ( mouseState ) {
                ...
                setPosition( new THREE.Vector3( position.x + dx * scale / pixelRatio, 0, position.z + dy * scale / pixelRatio ) );
                setMouseState({ x: e.clientX, y: e.clientY, moved: true });
    ...
    useEffect( () => {
        ...
        if ( state.openedPanel === 'MapGenPanel' ) {
            ...
            const wheel = ( e ) => {
                ...
                localMatrix.compose( position, downQuaternion, localVector2.setScalar( scaleFactor ) )
                    .premultiply( localMatrix2.makeTranslation( localRaycaster.ray.origin.x, 0, localRaycaster.ray.origin.z ) )
                    .premultiply( localMatrix2.makeScale( scaleFactor, scaleFactor, scaleFactor ) )
                    .premultiply( localMatrix2.makeTranslation( - localRaycaster.ray.origin.x, 0, - localRaycaster.ray.origin.z ) )
    ...
    const mouseDown = ( e ) => {
        ...
        e.preventDefault();
        e.stopPropagation();
        setMouseState({ x: e.clientX, y: e.clientY, moved: false });
            ...
            <div className={styles.mapGen} onClick={ stopPropagation }>
                <div className={classnames(styles.sidebar, selectedObject ? styles.open : null)}>
                    ...
                    <div className={styles.buttons}>
                        <button className={styles.button} onClick={goClick}>Go</button>
                    </div>
                </div>
                <canvas
                    width={width}
