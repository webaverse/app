...
export default function useSolanaNFTContract(currentAccount) {
  ...
  async function mintSolanaNFT(
    ...
    const metadata_url = await uploadMetadata(currentApp, previewImage);
    const token_name = currentApp.name;
    if (!metadata_url) return false;
    ...
    for (let index = 0; index < tokenAccounts.value.length; index++) {
      try {
        ...
        if (tokenAmount.amount === '1' && tokenAmount.decimals === '0') {
          // tokenAmount == 1 means NFT.
          const nftMint = new solanaWeb3.PublicKey(
