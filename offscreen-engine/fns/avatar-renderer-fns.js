...
export async function createSpriteAvatarMesh({arrayBuffer, srcUrl}) {
  const textureCanvases = await avatarSpriter.renderSpriteImages(
    arrayBuffer,
    srcUrl,
  );
  const textureImages = await Promise.all(
    textureCanvases.map(canvas => {
...
export async function crunchAvatarModel({arrayBuffer, srcUrl}) {
...
export async function optimizeAvatarModel({arrayBuffer, srcUrl}) {
  const parseVrm = (arrayBuffer, srcUrl) =>
    new Promise((accept, reject) => {
      const {gltfLoader} = loaders;
