...
let gizmoGltf = null;
export default class TransformGizmo extends THREE.Object3D {
  static async load() {
    if (gizmoGltf) {
  ...
  constructor() {
    ...
    this.selectionColor = new THREE.Color().setRGB(1, 1, 1);
...
(async () => {
  ...
  gizmo.setTransformMode(TransformMode.Translate);
  pe.scene.add(gizmo);
  /* const {GLTFLoader} = await import('./GLTFLoader.js');
  const o = await new Promise((accept, reject) => {
    new GLTFLoader().load('./TransformGizmo.glb', accept, function onProgress() {}, reject);
  });
  console.log('got scene', o.scene);
  const model = o.scene;

  pe.scene.add(o.scene); */
})();
