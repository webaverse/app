/*
totum uses plugins to load files from the metaverse and load them as apps.
...
totum.setApi({
  ...
  async load(u) {
    const m = await totum.import(u);
    const app = totum.createApp();
    await totum.addModule(app, m);
    return app;
  },
  useApp() {
  ...
  createApp({/* name = '', */start_url = '', type = '', /*components = [], */in_front = false} = {}) {
    ...
    if (start_url) {
      (async () => {
        const m = await totum.import(start_url);
        await totum.addModule(app, m);
      })();
    }
    app.addEventListener('destroy', () => {
...
const {apps, createApp, createModule, addApp, removeApp, useFrame, useLocalPlayer, getAppByName, getAppsByName, getAppsByType, getAppsByTypes, getAppsByComponent} = totum;
...
`;
  ...
  getAppByInstanceId() {
    const localPlayer = totum.useLocalPlayer();
    const remotePlayers = totum.useRemotePlayers();
