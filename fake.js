...
function importTextures() {
  ...
  let args = ['idle', 'walk', 'walk left', 'walk right', 'walk backward', 'run', 'run left', 'run right', 'run backward', 'crouch idle', 'crouch walk', 'crouch walk left', 'crouch walk right', 'crouch walk backward', 'naruto run','jump'];
  let textures = [];

  // load a resource
  for (let i = 0; i < args.length; i++) {
    loader.load(
      '/SpriteMesh/' + args[i] + '.png',
      function ( texture ) {
        texture.name = args[i];
  ...
  //console.log(textures, "texturessss");
  return textures;
}
function createSpriteMegaMesh(skinnedVrm) {
  let spriteImages = _renderSpriteImages(skinnedVrm);
  //spriteImages = importTextures();
