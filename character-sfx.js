...
// TODO : Create a new class called "AvatarCharacterSfx" which inherits from this class
export class CharacterSfx {
  constructor(character) {
    ...
    if (this.character.isLocalPlayer) {
      ...
      this.character.addEventListener('wearupdate', wearupdate);
      this.cleanup = () => {
        this.character.removeEventListener('wearupdate', wearupdate);
      };
    }
  ...
  update(timestamp, timeDiffS) {
    ...
    // step
    const _handleStep = () => {
      if (idleWalkFactor > 0.7 && !this.character.avatar.jumpState && !this.character.avatar.fallLoopState && !this.character.avatar.flyState && !this.character.hasAction('swim')) {
        ...
        const walkRunAnimationName = (() => {
          if (isNarutoRun) {
            ...
            const animationAngles = isCrouching ?
              AvatarAnimator.getClosest2AnimationAngles('crouch', this.character.avatar.getAngle())
            :
              (isRunning ?
                AvatarAnimator.getClosest2AnimationAngles('run', this.character.avatar.getAngle())
              :
                AvatarAnimator.getClosest2AnimationAngles('walk', this.character.avatar.getAngle())
              );
      ...
      if (useAction) {
        const _handleEat = () => {
          ...
          if (eatFrameIndex !== 0 && eatFrameIndex !== this.lastEatFrameIndex) {
            ...
            this.character.characterBehavior.setMouthMoving(0.04,0.04,0.1,0.02);
          }
        ...
        const _handleDrink = () => {
          ...
          if (drinkFrameIndex !== 0 && drinkFrameIndex !== this.lastDrinkFrameIndex) {
            ...
            // control mouth movement
            this.character.characterBehavior.setMouthMoving(0.1,0.1,0.1,0.1);
          }
