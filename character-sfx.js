...
class CharacterSfx {
  ...
  update(timestamp, timeDiffS) {
    ...
    const _handleJump = () => {
      ...
      } else if (this.lastJumpState && !this.player.avatar.jumpState && !this.player.avatar.swimState) {
    ...
    // step
    const _handleStep = () => {
      if (idleWalkFactor > 0.7 && !this.player.avatar.jumpState && !this.player.avatar.flyState && !this.player.avatar.swimState) {
