...
class TransformControls extends Object3D {
  ...
  constructor(camera, domElement, model) {
    ...
    const _gizmo = new TransformControlsGizmo(model);
    this._gizmo = _gizmo;
    this.add(_gizmo);
  ...
  pointerHover(raycaster) {
    ...
    const intersect = intersectObjectWithRay(this._gizmo.picker[this.mode], raycaster);
  ...
  pointerDown(_raycaster) {
    ...
    this.pointerDownHasAxis = this.axis !== null;

  }

  pointerMove(raycaster) {
...
function onPointerHover(raycaster) {

  if (!this.enabled) return;

  this.pointerHover(raycaster);
...
function onPointerDown(raycaster) {
  ...
  this.enabledPointerMove = true;

  this.pointerHover(raycaster);
  this.pointerDown(raycaster);

}

function onPointerMove(raycaster) {

  if (!this.enabled || !this.enabledPointerMove) return;

  this.pointerMove(raycaster);

}

function onPointerUp(raycaster) {

  if (!this.enabled) return;

  this.enabledPointerMove = false

  this.pointerUp(raycaster);
...
class TransformControlsGizmo extends Object3D {
  ...
  constructor(model) {
    ...
    this.translateControls = model.getObjectByName("TranslateControls");
    ...
    this.rotateControls = model.getObjectByName("RotateControls");
    ...
    const gizmoRotate = {
      XYZE: [
        [new Mesh(CircleGeometry(1, 1), matGray), null, [0, Math.PI / 2, 0]]
      ],
      X: [
        [rotateXAxisDisk]
      ],
      ...
      E: [
        [new Mesh(CircleGeometry(1.5, 1), matYellowTransparent), null, [0, Math.PI / 2, 0]]
      ]
    };
    ...
    const pickerRotate = {
      XYZE: [
        [new Mesh(new SphereGeometry(0.5, 10, 8), matInvisible)]
      ],
      X: [
        [rotateXAxisDisk]
      ],
      ...
      E: [
        [new Mesh(new TorusGeometry(1.5, 0.1, 2, 24), matInvisible)]
      ]
    };

    this.scaleControls = model.getObjectByName("ScaleControls");
    const scaleXAxis = this.scaleControls.getObjectByName("ScaleXAxis");
    const scaleYAxis = this.scaleControls.getObjectByName("ScaleYAxis");
    const scaleZAxis = this.scaleControls.getObjectByName("ScaleZAxis");
    const scaleXYPlane = this.scaleControls.getObjectByName("ScaleXYPlane");
    const scaleYZPlane = this.scaleControls.getObjectByName("ScaleYZPlane");
    const scaleXZPlane = this.scaleControls.getObjectByName("ScaleXZPlane");
