...
class CameraGeometry extends THREE.BufferGeometry {
  constructor() {
    ...
    const positions = new Float32Array(
      boxGeometry.attributes.position.array.length * 8,
    );
    ...
    const _setMatrixBetweenPoints = (m, p1, p2) => {
      const quaternion = localQuaternion.setFromRotationMatrix(
        localMatrix.lookAt(p1, p2, localVector.set(0, 1, 0)),
      );
      const position = localVector.copy(p1).add(p2).divideScalar(2);
    ...
    [
      ...
      const [p1, p2] = e;
      _pushBoxGeometry(_setMatrixBetweenPoints(localMatrix, p1, p2));
    });
