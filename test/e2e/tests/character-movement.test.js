...
describe(
  ...
  () => {
    beforeAll(async () => {
      ...
      // })
      await enterScene(
        `https://local.webaverse.com:3000/?src=.%2Fscenes%2Ftest-e2e-empty.scn`,
      );
    }, totalTimeout);
      ...
      async () => {
        ...
        const avatarFlag = await getCurrentPage().evaluate(async () => {
          ...
          const isCharacterSfx =
            localPlayer.characterSfx && !!localPlayer.characterSfx.player;
          const isCharacterHups =
            localPlayer.characterHups && !!localPlayer.characterHups.player;
          const isCharacterFx =
            localPlayer.characterFx && !!localPlayer.characterFx.player;
          const isCharacterHitter =
            localPlayer.characterHitter && !!localPlayer.characterHitter.player;
          const isCharacterBehavior =
            localPlayer.characterBehavior &&
            !!localPlayer.characterBehavior.player;
