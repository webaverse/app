...
const _getHorizontalBlend = (k, lerpFn, isPosition, target, avatar, now) => {
    _get7wayBlend(
        avatar,
        angles,
...
export const getActiveAnimation = (animation) => {
    return activeAnimations[animation];
}

export const _getIdleAnimation = key => animationsIdleArrays[key].animation;
...
export const _getAngleToBackwardAnimation = (angle, animation) => {
    const animationAngles = angles[animation].mirrored; //this in particular needs review
    const animations = animationAngles.map(({ animation }) => animation);
