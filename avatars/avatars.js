...
VRMSpringBone.prototype.update = (_update => {
  const getWorldQuaternionLite = (() => {
  ...
  return function(delta) {
    ...
    this._getMatrixWorldToCenter(localMatrix);
    localMatrix.multiply(this.bone.matrixWorld); // 🔥 ??
    this._centerSpacePosition.setFromMatrixPosition(localMatrix);

    // Get parent position in center space
    this._getMatrixWorldToCenter(localMatrix2);
    ...
    const initialCenterSpaceMatrixInv = localMatrix.copy(localMatrix2.multiply(this._initialLocalMatrix)).invert();
    const applyRotation = localQuaternion.setFromUnitVectors(
      this._boneAxis,
      localVector.copy(this._nextTail).applyMatrix4(initialCenterSpaceMatrixInv).normalize(),
    );
