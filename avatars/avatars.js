...
class Avatar {
  ...
  update(timeDiff) {
    ...
    const _applyWind = () => {
      ...
      if (windParameters.refreshTime === undefined) {
        windParameters.refreshTime = this.now;
      }
      if (windParameters.refreshDelay === undefined) {
        windParameters.refreshDelay = 100.0;
      }

      if (this.now > windParameters.refreshTime) {

        if (windParameters.forceFactor != 0 && windParameters.direction) {
          ...
          noiseX = (Math.abs(noiseX) * windParameters.direction.x);
          noiseY = (Math.abs(noiseY) * windParameters.direction.y);
          noiseZ = (Math.abs(noiseZ) * windParameters.direction.z);
          ...
          windParameters.refreshTime = this.now + windParameters.refreshDelay;
        }
        else {
          for(var i=0;i<this.springBoneManager.springBoneGroupList.length;i++) {
            for(var j=0;j<this.springBoneManager.springBoneGroupList[i].length;j++) {
          ...
          windParameters.refreshTime = this.now + 10000.0; // 10s
        }
