...
const _gameUpdate = (timestamp, timeDiff) => {
  ...
  const _updatePhysicsHighlight = () => {
    ...
    if (highlightedPhysicsObject) {
      ...
      const physicsObject = /*window.lolPhysicsObject ||*/ metaversefileApi.getAppByPhysicsId(physicsId).getPhysicsObject();
      if(physicsObject)
      {
  ...
  const _updateMouseHighlight = () => {
    ...
    if (h && !gameManager.dragging) {
      ...
      const physicsObject = metaversefileApi.getAppByPhysicsId(physicsId).getPhysicsObject();
      if (physicsObject) {
        const {physicsMesh} = physicsObject;
  ...
  if (crosshairEl) {
    const visible = !!document.pointerLockElement &&
      (['camera', 'firstperson', 'thirdperson'].includes(cameraManager.getMode()) || useLocalPlayer().hasAction('aim')) &&
      !_getGrabbedObject(0);
...
const gameManager = {
  ...
  isCrouched() {
    return useLocalPlayer().hasAction('crouch');
  },
  toggleCrouch() {
    const localPlayer = useLocalPlayer();
  ...
  isJumping() {
    return useLocalPlayer().hasAction('jump');
  },
  ensureJump() {
    const localPlayer = useLocalPlayer();
  ...
  isSitting() {
    return useLocalPlayer().hasAction('sit');
  },
  getMouseHoverObject() {
