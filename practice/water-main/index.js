  ...
  constructor({ procGenInstance, physics, biomeUvDataTexture, textures }) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      `,
        ...
        //       vec3 dhA = UnpackDerivativeHeight(texture2D(waterDerivativeHeightTexture, uvwA.xy * 1.5)) * uvwA.z * 0.15;
        //       vec3 dhB = UnpackDerivativeHeight(texture2D(waterDerivativeHeightTexture, uvwB.xy * 1.5)) * uvwB.z * 0.15;
        //       vec3 surfaceNormal = normalize(vec3(-(dhA.xy + dhB.xy), 1.));

        // vec3 fromSunVector = worldPosition.xyz - (sunPosition);
		// vec3 specularLight = vec3(0.0);
        // vec3 reflection = normalize( reflect( -fromSunVector, surfaceNormal ) );
        // float direction = max( 0.0, dot( eyeDirection, reflection ) );
        // specularLight += pow( direction, 500.0 ) * vec3(0.945, 0.970, 0.601) * 2.;
        ...
        //     //   vec3 specularHighlight = vec3(0.8, 0.8, 0.8) * specular * reflectivity;
                 
        //       vec4 texA = texture2D(waterNoiseTexture, uvwA.xy) * uvwA.z;
        //       vec4 texB = texture2D(waterNoiseTexture, uvwB.xy) * uvwB.z;
        //       gl_FragColor = (texA + texB) * vec4(0.048 / 1.8, 0.24 / 1.8, 0.384 / 1.8, 0.95) + vec4(0.0282, 0.431, 0.47, 0.);
        //     //   gl_FragColor.rgb /= 3.;
