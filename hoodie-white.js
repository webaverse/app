const imageSize = [2048, 2048];
const decalSize = 350;
const offset = [Math.floor((imageSize[0] - decalSize)/2), 1150];
...
(async () => {
  ...
  const backgroundImage = await loadImage('./hoodie-white.png');
        ...
        })();
        {
          ...
          for (let y = 0; y < imageData.height; y++) {
            for (let x = 0; x < imageData.width; x++) {
              ...
              for (let z = 0; z < 3; z++) {
                let f = imageData.data[x * 4 + (y * imageData.width * 4) + z];
                f = 255 - (((f/255)**0.1) * 255);
