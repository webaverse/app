...
onmessage = e => {
    ...
    if (e.data.message === 'generateTerrain') {
        ...
        const biomes = moduleInstance.HEAPF32.subarray(
            biomeBuffer / ELEMENT_BYTES + vertexRanges[2 * slots[0]] * 3,
            biomeBuffer / ELEMENT_BYTES + vertexRanges[2 * slots[0]] * 3 + vertexRanges[2 * slots[0] + 1] * 3
        );
        ...
        arrays[2].set(biomes, vertexRanges[2 * slots[0]] * 3);
